...
static CUresult createMyCtx(int dev_id) {
...
int main(int argc, char **argv) {
  printf("[ %s ]\n", sSDKsample);
	...
	for (int i = 0; i < 8; i++) {
		printf("create context on device %d start\n", i);
		createMyCtx(i);
		printf("======create context on device %d end\n", i);
...
static int initCUDA(int argc, char **argv, CUfunction *pMatrixMul) {
  ...
  if (module_path.rfind("ptx") != std::string::npos) {
    ...
    checkCudaErrors(cuModuleLoadDataEx(&cuModule, ptx_source.c_str(), jitNumOptions,
...
#endif
  {
    checkCudaErrors(cuModuleGetFunction(&cuFunction, cuModule, "matrixMul_bs32_32bit"));
  }
